% It is a simulator based on simulator2.m, we make it as close as possible
% to the simulations used in Taylor et al (2014) arXiv:1406.5224 (distP, sd)
% Oct. 28, 2014

tic;

% master directory for simulated data
simDataDir = 'simData9_snr0123_loc5';
...
% snr_net(1)=8.0*12.7/1.34;  % loc=4
% snr_net(2)=30.0*12.7/1.34;
% snr_net(3)=100.0*12.7/1.34;

snr_net(1)=8.0*12.7/1.34/1.54;  % loc=5
snr_net(2)=30.0*12.7/1.34/1.54;
snr_net(3)=100.0*12.7/1.34/1.54;
...
Np=9;  % number of pulsars in the timing array
...
pname={tmp1 tmp2 tmp3 tmp4 tmp5 tmp6 tmp7 tmp8 tmp9};
...
Nrlz=200;  %500;  %200;  %25;  % number of noise realizations H1
noise=zeros(Np,N);  % noise
%sd=0.1*10^(-7);  % standard deviation of the normal distribtion (sec)

Nnis=10; %500; %100;  % number of realization of noise only cases H0
