function [] = perfeval(simDataDir,searchParamsFile, algoType)
...
% A is a string that specifies which algorithm to use for the analysis.
%    A = 'mpp' : Uses maxPhase
%    A = 'avp' : Uses AvPhase

% For each input file, run PSO 8 times and record results. 
% For each input file, record an output file with:
%
%  * Path to input file.
...
%parfor lpFile = 1:nInputFiles
    ...
    switch algoType
        ...
        case 'avp'
            fHandle = @(x) LLR_PSOav(x, inParams);
        otherwise
            error(['Unrecognized Algorithm type: ',algoType]);
    end
...
%         psoResults=pso(fHandle,nDim,psoParams,outP);
        ...
        [~,bestLocRealC(lprun,:)] = fHandle(psoResults.bestLocation);
