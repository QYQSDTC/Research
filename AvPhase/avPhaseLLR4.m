...
function [LLR,varargout]=avPhaseLLR4(x,inParams)
...
for i=1:1:Np
    ...
    for j=1:1:4
        ...
        norm(j) = max(bs);
        %bn = bs;  %-norm(j);  % bn<=0
        fun=@(x)exp(sign(j,2)*bs(2).*cos(x)+sign(j,3)*bs(3).*sin(x) ...
            +sign(j,4)*bs(4).*sin(2*x)+sign(j,5)*bs(5).*cos(x).^2 ... 
            -4.0*norm(j));  % x=2*pulsar_phase
            %+sign(j,6)*bs(6).*sin(x).^2-5.0*norm(j));  % x=2*pulsar_phase
        LRn(j) = integral(fun,intlow(j),intup(j)) * exp(sign(j,1)*bs(1)-norm(j));  % normalized LR
        
        if isinf(LRn(j))
            disp(['avPhaseLLR: Inf for LRn at j= ',num2str(j)]);
            break;
        elseif isnan(LRn(j))
            disp(['avPhaseLLR: Nan for LRn at j= ',num2str(j)]);
            break;
        end

    end
    norm1=5.0 * norm + log(LRn);
    N=max(norm1);
