...
% 10/25/2016, YW, subtract the maximum value of b to avoid infinity in
% integration of the exponentials
...
for i=1:1:Np
    ...
    norm = max(b(i,:));
    b(i,:) = b(i,:)-norm;  % subtract the maximum to avoid Inf in integration
    
%     fun=@(x)exp(bn(2).*cos(x)+bn(3).*sin(x)+bn(4).*sin(x).*cos(x)...
%         +bn(5).*cos(x).^2+bn(6).*sin(x).^2);  % x=2*pulsar_phase
    
    fun=@(x)exp(b(i,2).*cos(x)+b(i,3).*sin(x)+b(i,4).*sin(x).*cos(x)...
        +b(i,5).*cos(x).^2+b(i,6).*sin(x).^2);  % x=2*pulsar_phase

%    LR = (integral(fun,0,2*pi) * exp(bn(1)))^norm;  % LR without normalization
    LR = integral(fun,0,2*pi) * exp(b(i,1));  % there should be an overall factor 1/2?
    LLR = LLR + log(LR) + 6.0*norm;  % LLR is additive, LR is not.
